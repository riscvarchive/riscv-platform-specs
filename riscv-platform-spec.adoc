...
Inbound accesses from PCIe devices may optionally be presented unmodified to an 
IOMMU for address tranlation. The unmodified physical address sent by a PCIe device when
no IOMMU was employed or a translated physical address provided by the IOMMU must 
be used unmodified for accessing system memory.
